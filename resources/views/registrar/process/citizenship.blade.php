@extends('layouts.registrar')

@section('title', 'PLP - Citizenship')
@section('page-title', 'CITIZENSHIP')

@section('content')
<div class="student-page-container">

    {{-- Code/Description Input Row + Add Button --}}
    <div class="registrar-form-row">
        <div class="registrar-form-group">
            <label class="registrar-form-label">CODE</label>
            <input type="text" class="registrar-form-input" placeholder="Code" name="code">
        </div>
        <div class="registrar-form-group" style="flex: 2;">
            <label class="registrar-form-label">DESCRIPTION</label>
            <input type="text" class="registrar-form-input" placeholder="Description" name="description">
        </div>
        <div class="registrar-form-group registrar-form-btn-group">
            <button type="button" class="btn-registrar-save" onclick="saveCitizenship()">+Add</button>
        </div>
    </div>

    {{-- Data Table --}}
    <div class="student-table-wrapper table-responsive">
        <table class="student-table registrar-table">
            <thead>
                <tr>
                    <th style="width: 50px;">#</th>
                    <th style="width: 120px;">Code</th>
                    <th>Description</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td style="width: 50px; text-align: center;"><input type="checkbox"></td>
                    <td style="width: 120px; text-align: center;">AFG</td>
                    <td style="text-align: left;">Afghan</td>
                </tr>
                <tr>
                    <td style="width: 50px; text-align: center;"><input type="checkbox"></td>
                    <td style="width: 120px; text-align: center;">ALB</td>
                    <td style="text-align: left;">Albanian</td>
                </tr>
                <tr>
                    <td style="width: 50px; text-align: center;"><input type="checkbox"></td>
                    <td style="width: 120px; text-align: center;">DZA</td>
                    <td style="text-align: left;">Algerian</td>
                </tr>
            </tbody>
        </table>
    </div>

</div>
@push('scripts')
<script>
function saveCitizenship() {
    var code = document.querySelector('input[name="code"]').value.trim();
    var desc = document.querySelector('input[name="description"]').value.trim();
    if (!code || !desc) {
        showRegistrarToast('Please fill in all required fields.', 'warning');
        return;
    }
    showRegistrarToast('Citizenship saved successfully.', 'success');
}
</script>
@endpush
@endsection
