<aside class="plp-sidebar">
    @php
        ...
        // Ensure we have the applicant object even if not passed to view
        $currentApplicant = $applicant ?? (auth()->check() ? auth()->user()->applicant : null);

        if ($currentApplicant) {
            $isRealPortalUnlocked = (
                optional($currentApplicant)->application_status === 'submitted'
            );
        }
    ...
    <nav class="sidebar-nav">
        ...
        {{-- Documents Submitted --}}
        <a href="{{ route('applicant.documents-submitted') }}" class="sidebar-link {{ request()->routeIs('applicant.documents-submitted') ? 'active' : '' }}">
        ...
        {{-- Medical Clearance --}}
        <a href="{{ route('applicant.medical-clearance') }}" class="sidebar-link {{ request()->routeIs('applicant.medical-clearance') ? 'active' : '' }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
