document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSectionDetails(section) {
        ...
        if (!rows.length) {
            ...
            soBody.innerHTML = rows.map(function (item) {
                ...
                return '' +
                    '<tr class="so-subject-row" data-code="' + escapeHtml(item.code) + '" style="cursor:pointer;" title="Click to edit subject details">' +
    ...
    if (soBody) {
        soBody.addEventListener('click', function (e) {
            ...
            var subject = (section.subjects || []).find(function(s) { return s.code === code; });
            if (!subject) return;
            ...
            if (soEditSubjectModal) {
                soEditSubjectModal.style.display = 'block';
            }
        });
    }
