document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSectionDetails(section) {
        ...
        if (!rows.length) {
            ...
            soBody.innerHTML = rows.map(function (item, index) {
                ...
                return '' +
                    '<tr class="so-subject-row" data-so-subject-index="' + index + '">' +
    ...
    function buildScheduleRowHtml(entry) {
        ...
        return ''
            + '<tr data-so-row-id="' + escapeHtml(getNextSubjectRowId()) + '">'
            + '<td><select class="app-filter-input so-subject-day">' + dayOptions + '</select></td>'
            + '<td class="so-subject-check-col"><input type="checkbox" class="so-subject-enabled" ' + ((entry && entry.enabled !== false) ? 'checked' : '') + '></td>'
            + '<td><input type="text" class="app-filter-input so-subject-time-from" value="' + escapeHtml(entry && entry.from || '') + '" placeholder="07:00AM"></td>'
            + '<td><input type="text" class="app-filter-input so-subject-time-to" value="' + escapeHtml(entry && entry.to || '') + '" placeholder="08:30AM"></td>'
            + '<td><input type="text" class="app-filter-input so-subject-room" value="' + escapeHtml(entry && entry.room || '') + '" placeholder="-select room-"></td>'
            + '<td class="so-subject-check-col"><input type="checkbox" class="so-subject-lab" ' + ((entry && entry.lab) ? 'checked' : '') + '></td>'
            + '<td class="so-subject-row-actions">'
            + '<button type="button" class="so-subject-row-btn so-subject-row-add" data-so-subject-row-action="add" aria-label="Add row">+</button>'
            + '<button type="button" class="so-subject-row-btn so-subject-row-remove" data-so-subject-row-action="remove" aria-label="Remove row">&times;</button>'
            + '</td>'
            + '</tr>';
    }
    ...
    function renderSectionConfigModal(section) {
        ...
        if (soSectionConfigMeta) {
            var metaRows = [
                { label: 'Course', value: normalizeText(section.program) || '-' },
                { label: 'Section', value: normalizeText(section.section) || '-' },
                { label: 'School Year', value: normalizeText(section.schoolYear) || '-' },
                { label: 'Semester', value: normalizeText(section.semester) || '-' },
                { label: 'Year Level', value: normalizeText(section.yearLevel) || '-' },
                { label: 'Adviser', value: normalizeText(section.adviser) || 'TBA' },
                { label: 'Slots', value: String(section.slots || 0) },
                { label: 'Subjects', value: String((section.subjects || []).length) }
            ];

            soSectionConfigMeta.innerHTML = metaRows.map(function (metaItem) {
                return '' +
                    '<div class="so-config-meta-item">' +
                    '<div class="so-config-meta-label">' + escapeHtml(metaItem.label) + '</div>' +
                    '<div class="so-config-meta-value">' + escapeHtml(metaItem.value) + '</div>' +
                    '</div>';
            }).join('');
        }
        ...
        soSectionConfigBody.innerHTML = rows.map(function (item, index) {
            ...
            return '' +
                ...
                '<td>' + escapeHtml(item.professor || 'TBA') + '</td>' +
                '<td>' + escapeHtml(item.slots || 0) + '</td>' +
                '<td class="so-config-schedule">' + (scheduleLines || '<div class="so-config-schedule-line">-</div>') + '</td>' +
                '<td class="so-config-actions">' +
                '<button type="button" class="so-config-action" data-so-config-action="configure" data-so-section-id="' + escapeHtml(section.id) + '" data-so-subject-index="' + index + '">Configure</button>' +
                '<button type="button" class="so-config-action so-config-action-edit" data-so-config-action="edit" data-so-section-id="' + escapeHtml(section.id) + '" data-so-subject-index="' + index + '">Edit</button>' +
                '</td>' +
                '</tr>';
        }).join('');
    }
    ...
    function readSubjectScheduleRows() {
        ...
        Array.prototype.forEach.call(rows, function (row) {
            ...
            var timePart = (fromValue !== '' || toValue !== '')
                ? ((fromValue || 'TBA') + '-' + (toValue || 'TBA'))
                : 'TBA';
