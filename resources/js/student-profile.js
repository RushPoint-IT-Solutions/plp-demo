...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var draft = (function () {
        try { var r = localStorage.getItem(DRAFT_KEY); return r ? JSON.parse(r) : null; } catch (e) { return null; }
    })();
    ...
    if (profilePhotoInput) {
        profilePhotoInput.addEventListener('change', function (e) {
            ...
            reader.onload = function (ev) {
                ...
                if (photoBtn) { photoBtn.textContent = 'Change Profile Picture'; }
                // Persist photo in draft
                try {
                    var d = {};
                    try { d = JSON.parse(localStorage.getItem(DRAFT_KEY) || '{}'); } catch (ex) {}
    ...
    function setupPhoneField(el) {
        el.addEventListener('input', function () {
            ...
            if (digits.length === 1 && digits !== '0') { digits = '0' + digits; }
            this.value = digits.substring(0, 11);
        });
    ...
    function setupOtherSwap(selectEl, wrapEl, inputEl, backBtnEl) {
        if (!selectEl || !wrapEl || !inputEl || !backBtnEl) { return; }
    ...
    setupOtherSwap(
        document.getElementById('incomeSourceSelect'),
        document.getElementById('incomeSourceOtherWrap'),
        document.getElementById('incomeSourceOtherInput'),
        document.getElementById('incomeSourceBackBtn')
    );
    setupOtherSwap(
        document.getElementById('livingSituationSelect'),
        document.getElementById('livingSituationOtherWrap'),
        document.getElementById('livingSituationOtherInput'),
        document.getElementById('livingSituationBackBtn')
    );
    setupOtherSwap(
        document.getElementById('lmsUsedSelect'),
        document.getElementById('lmsUsedOtherWrap'),
        document.getElementById('lmsUsedOtherInput'),
        document.getElementById('lmsUsedBackBtn')
    );
    setupOtherSwap(
        document.getElementById('lmsPreferredSelect'),
        document.getElementById('lmsPreferredOtherWrap'),
        document.getElementById('lmsPreferredOtherInput'),
        document.getElementById('lmsPreferredBackBtn')
    );
    ...
    if (sameAsPresent) {
        sameAsPresent.addEventListener('change', function () {
            ...
            if (sameAsPresent.checked) {
                ...
                if (xRegion)   { xRegion.disabled   = true; }
                if (xProvince) { xProvince.disabled  = true; }
                if (xCity)     { xCity.disabled      = true; }
            } else {
    ...
    function restoreDraft() {
        ...
        form.querySelectorAll('input, select, textarea').forEach(function (f) {
            ...
            if (f.type === 'checkbox' || f.type === 'radio') {
                ...
                if (draft[key] !== undefined) { f.checked = draft[key]; }
            } else {
                if (draft[f.name] !== undefined && draft[f.name] !== '') { f.value = draft[f.name]; }
            }
        });
        // Re-trigger age calculation
        if (dobField && dobField.value) { dobField.dispatchEvent(new Event('change')); }
        ...
        if (draft['__profilePhotoUrl']) {
            ...
            if (pImg)  { pImg.src = draft['__profilePhotoUrl']; pImg.style.display = 'block'; }
            if (pIcon) { pIcon.style.display = 'none'; }
            if (pBtn)  { pBtn.textContent = 'Change Profile Picture'; }
        } else if (draft['__profilePhoto']) {
            var pImg  = document.getElementById('profilePhotoImg');
            var pIcon = document.querySelector('.profile-photo-icon');
            var pBtn  = document.querySelector('.profile-photo-btn');
            if (pImg)  { pImg.src = draft['__profilePhoto']; pImg.style.display = 'block'; }
            if (pIcon) { pIcon.style.display = 'none'; }
            if (pBtn)  { pBtn.textContent = 'Change Profile Picture'; }
        }
        ...
        if (draft['__religionOther'] && religionSelect && religionOtherWrap) {
            religionSelect.style.display    = 'none';
            religionOtherWrap.style.display = 'block';
            if (religionOtherInput) { religionOtherInput.required = true; }
        }
        // Restore nationality Other state
        if (draft['__nationalityOther'] && nationalitySelect && nationalityOtherWrap) {
            nationalitySelect.style.display    = 'none';
            nationalityOtherWrap.style.display = 'block';
            if (nationalityOtherInput) { nationalityOtherInput.required = true; }
        }
        ...
        function _restoreOtherSwap(selId, wrapId, inputId, flag) {
            ...
            if (sel && wrap && input) {
                ...
                wrap.style.display = 'block'; input.required = true;
            }
        }
        ...
        if (devOtherCb && devOtherCb.checked && devOtherIn) { devOtherIn.style.display = 'block'; }
        var lmsRCb = document.querySelector('[name="lms_reasons[]"][value="Others"]');
        var lmsRIn = document.querySelector('[name="lms_reasons_other"]');
        if (lmsRCb && lmsRCb.checked && lmsRIn) { lmsRIn.style.display = 'block'; }
        updateAddressPreview();
    }
    ...
    var optionalNames = [
        'suffix', 'profile_photo', 'age', 'religion_other', 'nationality_other',
        'mother_contact', 'father_contact',
        'lrn',                               // nullable when no_k12 is toggled
        'family_income_source_other', 'living_situation_other',
        'lms_used_other', 'lms_preferred_other',
        'devices_other', 'lms_reasons_other'
    ];
    var seenRadioGroups = {};
